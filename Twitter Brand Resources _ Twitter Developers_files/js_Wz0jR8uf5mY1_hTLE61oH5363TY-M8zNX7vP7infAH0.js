 ...
 *
 * @see https://github.com/angular/angular.js/blob/www/v1.4.4/src/ng/urlUtils.js
 * @see https://grack.com/blog/2009/11/17/absolutizing-url-in-javascript
 * @see https://github.com/jquery/jquery-ui/blob/www/1.11.4/ui/tabs.js#L53
 */
Drupal.absoluteUrl=function(e){var t=document.createElement("a");// Decode the URL first; this is required by IE <= 6. Decoding non-UTF-8
// strings may throw an exception.
try{e=decodeURIComponent(e)}catch(n){}// IE <= 7 normalizes the URL when assigned to the anchor node similar to
// the other browsers.
 ...
 *
 * @see https://github.com/jquery/jquery-ui/blob/www/1.11.4/ui/tabs.js#L58
 */
Drupal.urlIsLocal=function(e){// Always use browser-derived absolute URLs in the comparison, to avoid
// attempts to break out of the base path using directory traversal.
var t=Drupal.absoluteUrl(e),n=location.protocol;// Consider URLs that match this site's base URL but use HTTPS instead of HTTP
// as local as well.
"http:"===n&&0===t.indexOf("https:")&&(n="https:");var r=n+"//"+location.host+Drupal.settings.basePath.slice(0,-1);// Decoding non-UTF-8 strings may throw an exception.
try{t=decodeURIComponent(t)}catch(i){}try{r=decodeURIComponent(r)}catch(i){}// The given URL matches the site's base URL, or has a path under the site's
// base URL.
return t===r||0===t.indexOf(r+"/")},/**
 ...
 */
Drupal.ajaxError=function(t,n,r){var i,o,a,s,u,l;i=t.status?"\n"+Drupal.t("An AJAX HTTP error occurred.")+"\n"+Drupal.t("HTTP Result Code: !status",{"!status":t.status}):"\n"+Drupal.t("An AJAX HTTP request terminated abnormally."),i+="\n"+Drupal.t("Debugging information follows."),a="\n"+Drupal.t("Path: !uri",{"!uri":n}),o="";// In some cases, when statusCode == 0, xmlhttp.statusText may not be defined.
// Unfortunately, testing for it with typeof, etc, doesn't seem to catch that
// and the test causes an exception. So we need to catch the exception here.
try{o="\n"+Drupal.t("StatusText: !statusText",{"!statusText":e.trim(t.statusText)})}catch(c){}s="";// Again, we don't have a way to know for sure whether accessing
// xmlhttp.responseText is going to throw an exception. So we'll catch it.
try{s="\n"+Drupal.t("ResponseText: !responseText",{"!responseText":e.trim(t.responseText)})}catch(c){}// Make the responseText more readable by stripping HTML tags and newlines.
// We don't need readyState except for status == 0.
// Additional message beyond what the xmlhttp object provides.
return s=s.replace(/<("[^"]*"|'[^']*'|[^'">])*>/gi,""),s=s.replace(/[\n]+\s+/g,"\n"),u=0==t.status?"\n"+Drupal.t("ReadyState: !readyState",{"!readyState":t.readyState}):"",r=r?"\n"+Drupal.t("CustomMessage: !customMessage",{"!customMessage":r}):"",l=i+a+o+r+s+u},// Class indicating that JS is enabled; used for styling purpose.
 ...
 */
Drupal.ajax=Drupal.ajax||{},Drupal.settings.urlIsAjaxTrusted=Drupal.settings.urlIsAjaxTrusted||{},/**
...
this.url=n.url.replace(/\/nojs(\/|$|\?|&|#)/g,"/ajax$1"),// If the 'nojs' version of the URL is trusted, also trust the 'ajax' version.
Drupal.settings.urlIsAjaxTrusted[n.url]&&(Drupal.settings.urlIsAjaxTrusted[this.url]=!0),this.wrapper="#"+n.wrapper,// If there isn't a form, jQuery.ajax() will be used instead, allowing us to
// bind Ajax to links as well.
this.element.form&&(this.form=$(this.element.form));// Set the options for the ajaxSubmit function.
// The 'this' variable will not persist inside of the options object.
var i=this;i.options={url:i.url,data:i.submit,beforeSerialize:function(e,t){return i.beforeSerialize(e,t)},beforeSubmit:function(e,t,n){return i.ajaxing=!0,i.beforeSubmit(e,t,n)},beforeSend:function(e,t){return i.ajaxing=!0,i.beforeSend(e,t)},success:function(e,t,n){// Prior to invoking the response's commands, verify that they can be
...
if(// Sanity check for browser support (object expected).
// When using iFrame uploads, responses must be returned as a string.
"string"==typeof e&&(e=$.parseJSON(e)),null!==e&&!Drupal.settings.urlIsAjaxTrusted[i.url]&&"1"!==n.getResponseHeader("X-Drupal-Ajax-Token")){var r=Drupal.t("The response failed verification so will not be processed.");return i.error(n,i.url,r)}return i.success(e,t)},complete:function(e,t){return i.ajaxing=!1,"error"==t||"parsererror"==t?i.error(e,i.url):void 0},dataType:"json",type:"POST"},// Bind the ajaxSubmit function to the element event.
$(i.element).bind(n.event,function(e){if(!Drupal.settings.urlIsAjaxTrusted[i.url]&&!Drupal.urlIsLocal(i.url))throw new Error(Drupal.t("The callback URL is not local and not trusted: !url",{"!url":i.url}));return i.eventResponse(this,e)}),// If necessary, enable keyboard submission so that Ajax behaviors
 ...
 */
Drupal.ajax.prototype.error=function(e,t,n){// Reattach behaviors, if they were detached in beforeSerialize().
if(alert(Drupal.ajaxError(e,t,n)),// Remove the progress element.
...
$(this.element).removeClass("progress-disabled").removeAttr("disabled"),this.form){var r=this.settings||Drupal.settings;Drupal.attachBehaviors(this.form,r)}},/**
...
var C=!1;try{C=Date.prototype.toJSON&&null===new Date(0/0).toJSON()&&-1!==new Date(D).toJSON().indexOf(E)&&Date.prototype.toJSON.call({// generic
...
var L="	\n\f\r   ᠎             　\u2028\u2029﻿";if(!String.prototype.trim||L.trim()){// http://blog.stevenlevithan.com/archives/faster-trim-javascript
// http://perfectionkills.com/whitespace-deviations/
L="["+L+"]";var _=new RegExp("^"+L+L+"*"),F=new RegExp(L+L+"*$");String.prototype.trim=function(){if(void 0===this||null===this)throw new TypeError("can't convert "+this+" to object");return String(this).replace(_,"").replace(F,"")}}// ES-5 15.1.2.2
(8!==parseInt(L+"08")||22!==parseInt(L+"0x16"))&&(parseInt=function(e){var t=/^0[xX]/;return function(n,r){return n=String(n).trim(),+r||(r=t.test(n)?16:10),e(n,r)}}(parseInt));// ES5 9.9
...
s();for(n in e.options.customVars)t.setCustomVar(n,e.options.customVars[n]);1==e.options.displayAdvertisingFeatures?l(function(e,t,n){ga(n+".require","displayfeatures")}):"undefined"!=typeof console&&console.log&&console.log("Tailored ads opt-out is enabled."),u()};f(e)},window.TwtrAnalyticsGoogle=e}(),function(){"use strict";var e;e=function(e){var t,n,r,i=null;/**
     ...
     * Callback is only triggered if the event is valid.
     */
this.trackEvents=function(e,n){var r,o,s;for(r=0;r<e.length;r++)if(this.isEventValid(e[r])!==!1){if(o={client:t,page:e[r].page,section:e[r].section,component:e[r].component,element:e[r].element,action:e[r].action},s={},s.terms=o,null!==i){var u=i();s.ref=u.ref}a(s,n)}else console.warn("Could not scribe event - page, section, component, element and/or action parameters are missing.")},/**
     * Check if a single event is valid
     */
